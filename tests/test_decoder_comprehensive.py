...
class TestEVCBatchDecoder:
    ...
    def test_fetch_vault_metadata_with_web3_multicall_success(self, mock_console, decoder, mock_web3):
        ...
        mock_contract.functions.aggregate3.return_value.call.return_value = [
            (True, b'Test Vault'),
            (True, b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x12\x34\x56\x78\x90\x12\x34\x56\x78\x90\x12\x34\x56\x78\x90\x12\x34\x56\x78\x90')
        ]
    ...
    def test_analyze_batch_with_nested_batch(self, decoder):
        ...
        nested_batch = BatchDecoding(items=[
            BatchItem(
        ...
        main_batch = BatchDecoding(items=[
            BatchItem(
                target_contract="0x1234567890123456789012345678901234567890",
                data="0x0ac3e31803e80320",
    ...
    def test_analyze_batch_router_governance_functions(self, decoder):
        ...
        batch = BatchDecoding(items=[
            BatchItem(
                target_contract="0x1234567890123456789012345678901234567890",
                data="0x2c4e0a11",
                decoded={"functionName": "govSetConfig", "args": {"base": "0x123", "quote": "0x456", "oracle": "0x789"}}
            )
        ])
    ...
    def test_format_output_with_timelock(self, mock_console, decoder):
        ...
        analysis = {"total_items": 1, "governance_operations": [], "vault_changes": {}, 
                   "router_changes": {}, "unknown_operations": [], "nested_batches": 0}
...
class TestDataClasses:
    ...
    def test_batch_decoding_creation(self):
        ...
        items = [
            BatchItem(target_contract="0x123", data="0xabcd")
        ]
