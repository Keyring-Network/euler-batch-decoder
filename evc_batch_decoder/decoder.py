...
class BatchItem:
    ...
    nested_batch: Optional['BatchDecoding'] = None
...
class BatchDecoding:
    ...
    timelock_info: Optional[TimelockInfo] = None
...
class EVCBatchDecoder:
    ...
    def __init__(self, chain_id: int = 43114):  # Default to Avalanche
        ...
        self.metadata = {}  # Will be populated dynamically
        self.governance_functions = {
            ...
            'govSetResolvedVault', 'govSetFallbackOracle'
        }
    ...
    def _load_function_signatures(self) -> Dict[str, Dict[str, Any]]:
        ...
        signatures = {
            ...
            "0x8bcd4016": {  # setInterestRateModel
                ...
                "inputs": [
                    {"name": "newInterestRateModel", "type": "address"}
                ]
            },
            "0x8d8fe2c3": {
                "name": "setGovernorAdmin", 
                "inputs": [{"name": "newGovernorAdmin", "type": "address"}]
            },
            "0xefdcd974": {
                "name": "setFeeReceiver",
            ...
            "0xd5a8b4a1": {
                "name": "setInterestRateModel",
                "inputs": [{"name": "newModel", "type": "address"}]
            },
            "0x0e32cb86": {
                "name": "setMaxLiquidationDiscount",
                "inputs": [{"name": "newDiscount", "type": "uint16"}]
            },
            "0x7b0472f0": {
                "name": "setHookConfig",
                "inputs": [
                    {"name": "newHookTarget", "type": "address"},
                    {"name": "newHookedOps", "type": "uint32"}
                ]
            },
            "0x6a1db1bf": {
                "name": "setInterestFee",
    ...
    def fetch_vault_metadata(self, vault_addresses: List[str], w3_client=None) -> None:
        """Fetch metadata for vault addresses using Multicall3 (like TG function from JS)."""
        if not vault_addresses:
            return
            
        if not w3_client:
            console.print(f"[yellow]Warning: Web3 client not provided, skipping metadata fetch[/yellow]")
            ...
            for address in vault_addresses:
                ...
                self.add_contract_metadata(address, {
                    "name": f"EVK Vault {short_addr}",
                    "type": "vault"
                })
            return
        ...
        try:
            ...
            if calls:
                ...
                for i in range(0, len(results), 2):
                    ...
                    asset_result = results[i + 1] if i + 1 < len(results) else None
                    
                    if name_result[0]:  # success
                        try:
                            # Decode string result (name)
                            from eth_abi import decode
                            vault_name = decode(['string'], name_result[1])[0]
                        except:
                    ...
                    # Store metadata
                    self.add_contract_metadata(vault_addr, {
                        "name": vault_name,
                        "type": "vault",
                        "kind": "vault"
                    })
            ...
            for address in vault_addresses:
                self.add_contract_metadata(address, {
                    "name": f"EVK Vault {address[:8]}...",
                    "type": "vault"
                })

    def fetch_router_metadata(self, router_addresses: List[str], w3_client=None) -> None:
        """Fetch metadata for router addresses using on-chain calls (like SG function from JS)."""
        if not router_addresses:
            return
            
        if not w3_client:
            console.print(f"[yellow]Warning: Web3 client not provided, skipping router metadata fetch[/yellow]")
        ...
        for address in router_addresses:
            ...
            short_addr = f"{address[:6]}...{address[-6:]}" if len(address) >= 12 else address
            self.add_contract_metadata(address, {
                "name": f"Oracle Router {short_addr}",
                "type": "router"
            })

    def fetch_oracle_metadata(self, oracle_addresses: List[str], w3_client=None) -> None:
        """Fetch metadata for oracle addresses using on-chain calls (like BG function from JS)."""
        if not oracle_addresses:
            return
            
        if not w3_client:
            console.print(f"[yellow]Warning: Web3 client not provided, skipping oracle metadata fetch[/yellow]")
        ...
        for address in oracle_addresses:
            ...
            short_addr = f"{address[:6]}...{address[-6:]}" if len(address) >= 12 else address
            self.add_contract_metadata(address, {
                "name": f"Oracle {short_addr}",
                "type": "oracle"
            })

    def decode_batch_data(self, data: Union[str, bytes, dict]) -> BatchDecoding:
    ...
    def _decode_batch_function(self, calldata: str) -> BatchDecoding:
        ...
        try:
            ...
            decoded_data = eth_decode(['(address,address,uint256,bytes)[]'], calldata_bytes)[0]
    ...
    def _decode_function_call(self, data: bytes) -> Optional[Dict[str, Any]]:
        ...
        if selector_with_prefix in self.function_signatures:
            ...
            try:
                ...
                if inputs:
                    ...
                    decoded_args = eth_decode(input_types, data[4:])
    ...
    def analyze_batch(self, batch_decoding: BatchDecoding, w3_client=None) -> Dict[str, Any]:
        ...
        def collect_addresses_from_items(items):
            for item in items:
                ...
                if item.decoded:
                    ...
                    if func_name in ['setCaps', 'setGovernorAdmin', 'setFeeReceiver', 
                                   'setInterestRateModel', 'setMaxLiquidationDiscount', 
                                   'setHookConfig', 'setInterestFee', 'setLiquidationCoolOffTime', 'setLTV']:
                        vault_addresses.add(item.target_contract)
                        
                    elif func_name in ['govSetConfig', 'transferGovernance', 'govSetResolvedVault', 'govSetFallbackOracle']:
        ...
        for i, item in enumerate(batch_decoding.items):
            if item.nested_batch:
                analysis['nested_batches'] += 1
                # Recursively analyze nested batch
                nested_analysis = self.analyze_batch(item.nested_batch)
                analysis['governance_operations'].extend(nested_analysis['governance_operations'])
                
            if item.decoded:
                func_name = item.decoded.get('functionName', 'unknown')
                
                if func_name in self.governance_functions:
                    analysis['governance_operations'].append({
                        'index': i,
                        'function': func_name,
                    ...
                    if func_name in ['setCaps', 'setGovernorAdmin', 'setFeeReceiver', 
                                   'setInterestRateModel', 'setMaxLiquidationDiscount', 
                                   'setHookConfig', 'setInterestFee', 'setLiquidationCoolOffTime', 'setLTV']:
                        if item.target_contract not in analysis['vault_changes']:
                            analysis['vault_changes'][item.target_contract] = []
                        analysis['vault_changes'][item.target_contract].append({
                            'function': func_name,
                            'args': item.decoded.get('args', {})
                        })
                        
                    elif func_name in ['govSetConfig', 'transferGovernance', 'govSetResolvedVault', 'govSetFallbackOracle']:
                        if item.target_contract not in analysis['router_changes']:
                            analysis['router_changes'][item.target_contract] = []
                        analysis['router_changes'][item.target_contract].append({
                            'function': func_name,
                            'args': item.decoded.get('args', {})
                        })
                        
                elif func_name == 'unknown':
                    analysis['unknown_operations'].append({
                        'index': i,
                        'target': item.target_contract,
    ...
    def format_readme_style(self, batch_decoding: BatchDecoding, analysis: Dict[str, Any]) -> str:
        ...
        for item in batch_decoding.items:
            if item.decoded:
                ...
                output.append(f"- {target_link} `.{func_name}({args_str})`, onBehalfOf={behalf_link} , value={item.value}")
    ...
    def format_output(self, batch_decoding: BatchDecoding, analysis: Dict[str, Any]) -> None:
        ...
        console.print(Panel.fit(
            "[bold blue]🧙‍♂️ EVC Batch Decoder Results[/bold blue]",
            border_style="blue"
        ))
